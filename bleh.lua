...
Version: 2.1 (Fixed String Formatting)
...
-- Drawing Container (Attempts GetRobloxGui, falls back to CoreGui)
local function getDrawingContainer()
    ...
    local success, hui = pcall(GuiService.GetRobloxGui, GuiService)
...
local function parseColor(value, defaultColor, defaultTransparency)
    if type(value) == "table" then
        return value[1] or defaultColor, value[2] ?? defaultTransparency -- Use ?? for Luau nil check
...
function InstanceObject:_InitializeDefaults()
    ...
    opt.Format = opt.Format or "{Name}\n[{Distance}m]" -- More informative default using 'm' for meters/studs
    opt.Color = opt.Color or { VisibleColor = {Color3.new(1,1,1), 0}, OccludedColor = {Color3.new(0.8,0.8,0.8), 0.1} } -- Visible/Occluded
    opt.Outline = opt.Outline or { Enabled = true, Color = {Color3.new(0,0,0), 0}, Thickness = 1 } -- Thinner default outline
...
local EspInterface = {
    ...
    _InstanceCleanupConnections = {}, -- Stores AncestryChanged connections for instances keyed by instance
    ...
    teamSettings = {
        Enemy = {
            ...
            Box2D = {
				...
				Fill = { Enabled = false, TransparencyModifier = 0.6, VisibleColor = { Color3.fromRGB(255, 50, 50), 0 }, OccludedColor = { Color3.fromRGB(200, 50, 50), 0.3 } }, -- Fill needs its own color settings now
            },
             -- == Box 3D ==
            Box3D = {
            ...
            -- == Text Elements (Make sure Format uses {Placeholder} syntax) ==
            NameText = {
                Enabled = true, VPadding = 2, Size = 14, Font = Enum.Font.GothamSemibold, Format = "{Name}", -- Added Format default
			...
			HealthText = {
				Enabled = false, AttachToBar = true, HPadding = 4, Size = 11, Font = Enum.Font.GothamBold, Format = "{Health}HP", -- Default format
				VisibleColor = { Color3.new(1,1,1), 0 }, OccludedColor = { Color3.new(0.8, 0.8, 0.8), 0.1 }, IgnoreTeamColor = true,
                Outline = { Enabled = true, Color = { Color3.new(0,0,0), 0 } }
			},
            WeaponText = {
                Enabled = false, VPadding = 0, Size = 12, Font = Enum.Font.Gotham, Format = "{Weapon}", -- Default format
                VisibleColor = { Color3.new(1,1,1), 0 }, OccludedColor = { Color3.new(0.8, 0.8, 0.8), 0.1 }, IgnoreTeamColor = true,
                Outline = { Enabled = true, Color = { Color3.new(0,0,0), 0 } }
            },
            -- == Tracer ==
        ...
        Friendly = { -- Inherits from Enemy by default, overrides below
            ...
            DistanceText = { Enabled = false, Format = "[{Distance}m]" }, -- Disabled for friends usually
			HealthText = { Enabled = false, Format = "{Health}HP" },
            WeaponText = { Enabled = false, Format = "{Weapon}" },
	...
	_DeepCopy = function(original)
		...
		for k, v in pairs(original) do
			if type(v) == "table" then
				copy[k] = EspInterface._DeepCopy(v) -- Recurse for nested tables
			else
				copy[k] = v
			end
		end
	...
	_DeepMerge = function(destination, source)
		for k, v in pairs(source) do
			if type(v) == "table" and type(destination[k]) == "table" then
				EspInterface._DeepMerge(destination[k], v) -- Recurse merge
			else
				destination[k] = v -- Overwrite or add primitive/new table
			end
		end
		return destination
	end,
    ...
    _CreatePlayerObjects = function(player)
		if not player or player == LocalPlayer then return end -- Don't make for self or invalid
		...
		if successEsp and successCham and espObj and chamObj then
			...
			if espObj then pcall(espObj.Destruct, espObj) end -- Cleanup partial creation
	...
	_SetupInstanceCleanup = function(instance, objRef)
		...
		if EspInterface._InstanceCleanupConnections[instance] then
			EspInterface._InstanceCleanupConnections[instance]:Disconnect() -- Disconnect old if any
		end
		-- Use weak key table? For now, manual disconnect on RemoveInstance is main cleanup
		EspInterface._InstanceCleanupConnections[instance] = instance.AncestryChanged:Connect(function(_, parent)
			if not parent then -- Instance removed from game
				...
				if EspInterface._ObjectCache[instance] then -- Check if we still have it cached
    ...
    Load = function()
		...
		-- Process Friendly settings: Start with a deep copy of Enemy, then merge Friendly overrides
		...
		-- Clear existing objects just in case Unload wasn't called properly
		EspInterface.Unload(true) -- Pass silent=true to avoid unload message
    ...
    Unload = function(silent)
		...
		-- clear(EspInterface._PlayerConnections) -- No need to clear if setting to nil

		-- Disconnect instance cleanup signals and clear table
		...
		-- if DrawingContainer and DrawingContainer.Parent then pcall(DrawingContainer.Destroy, DrawingContainer) DrawingContainer = nil end
    ...
    UpdateSetting = function(category, settingName, value)
		...
		-- Example: EspInterface.UpdateSetting("Enemy", "Box2D", { Enabled = false }) -- Deep merge nested table
		local targetTable
	...
	Toggle = function(enabled) -- Simple master toggle
		...
		-- For instance ESP, you'd likely iterate _ObjectCache and set obj.Options.Enabled
	end
}

-- Ensure the DrawingContainer exists on load
DrawingContainer = getDrawingContainer()

-- Pre-calculate friendly settings on script initialization (before Load is called)
local enemySettingsCopy = EspInterface._DeepCopy(EspInterface.teamSettings.Enemy)
EspInterface.teamSettings.Friendly = EspInterface._DeepMerge(enemySettingsCopy, EspInterface.teamSettings.Friendly)
